  ...
  if (cart.length === 0) {
    return (
      <div className="min-h-screen bg-brand-dark text-white flex flex-col justify-center items-center p-8 select-none">
        <h2 className="font-londrina text-5xl mb-4">Votre plateau est vide</h2>
        <button
    ...
    <div className="min-h-screen bg-brand-dark text-white p-6 sm:p-12 select-none flex flex-col justify-between h-screen overflow-hidden">
      ...
      <div className="flex justify-between items-end border-b border-white/10 pb-6 shrink-0">
        <div>
          <h2 className="font-londrina text-5xl tracking-wide uppercase">
            Vérifiez votre plateau
          </h2>
          <p className="text-amber-400 font-mono text-sm uppercase tracking-wider mt-1">
            Commande <span className="font-bold font-sans">#{orderNumber}</span>{" "}
            pour <span className="text-white font-bold">{customerName}</span> ·{" "}
            ...
            >
              <div className="space-y-1 flex-1">
                ...
                <button
                  ...
                  className="inline-block mt-3 font-londrina text-sm text-amber-500/80 border border-amber-900/40 bg-amber-950/10 px-4 py-1.5 rounded-xl transition-colors cursor-pointer"
                >
      ...
      <div className="bg-white text-brand-dark p-6 rounded-[2.5rem] shadow-2xl flex flex-col sm:flex-row justify-between items-center gap-6 shrink-0">
        ...
        <div className="flex gap-4 w-full sm:w-auto">
          <button
            onClick={() => setScreen("menu")}
            className="flex-1 sm:flex-initial px-8 py-4 border-2 border-brand-dark/10 text-brand-dark font-londrina text-xl rounded-2xl cursor-pointer active:scale-98 transition-transform bg-transparent"
          >
            Ajouter une autre boisson
          </button>
          <button
            onClick={() => setScreen("checkout")}
            className="flex-1 sm:flex-initial px-12 py-4 bg-brand-dark text-white font-londrina text-2xl rounded-2xl shadow-lg active:scale-98 transition-transform cursor-pointer"
          >
            Passer au paiement ➔
          </button>
        </div>
